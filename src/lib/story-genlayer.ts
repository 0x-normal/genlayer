...
const GAME = "story_wars" as const;
...
export interface StoryJudgingResult {
  prompt_combo: string;
  results: StoryResult[];
  winner: string;
  engine?: string;
...
export type StoryRoundMeta = { round: number; prompt_combo: string };
...
function isValidResult(obj: any): obj is StoryJudgingResult {
...
function extractResult(tx: unknown): StoryJudgingResult | null {
  return extractFromReceipt<StoryJudgingResult>(tx, isValidResult);
}
...
export async function submitEntry(
  ...
  console.log("[story submitEntry]", { gameId, round, txHash });
  return txHash;
}

export async function finalizeStories(
  callerAddress: string,
  gameId: string,
  roundsMeta: StoryRoundMeta[],
  allAddresses: string[],
): Promise<StoryJudgingResult> {
  ...
  console.log(`[finalizeStories] entries seen on-chain: ${lastSeen}/${expectedTotal}`);

  const txHash = await client.writeContract({
    address: CONTRACT(),
    functionName: "finalize_stories",
    args: [gameId, JSON.stringify(roundsMeta), currentWeek],
    value: BigInt(0),
    leaderOnly: true,
  });
  console.log("[finalizeStories] tx:", txHash);
  ...
  console.warn("[finalizeStories] Could not extract scores — using XP snapshot");
  ...
  const promptCombo = roundsMeta[0]?.prompt_combo ?? "";
  const results: StoryResult[] = ranked.map((r, i) => ({
    address: r.address,
    storytelling: 0,
    economy_of_words: 0,
    emotion: 0,
  ...
  return { prompt_combo: promptCombo, results, winner: results[0]?.address ?? "" };
}
