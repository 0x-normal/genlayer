import type { ExcuseJudgingResult, ExcuseResult } from "./excuse-genlayer";

export function judgeExcusesLocally(
  crime: string,
  rounds: { round: number; crime: string; excuses: { address: string; excuse: string }[] }[]
): ExcuseJudgingResult {
  ...
  for (const round of rounds) {
    for (const p of round.excuses) {
      const key = p.address.toLowerCase();
      const existing = playerMap.get(key);
      if (existing) {
        existing.allText += " " + p.excuse;
      } else {
        playerMap.set(key, { address: p.address, allText: p.excuse });
      }
    }
  }
  ...
  const results: ExcuseResult[] = players.map((p) => {
