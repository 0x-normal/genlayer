...
const GAME = "pitch_wars" as const;
...
export type PitchRoundMeta = { round: number; combo: string };
...
export async function submitEntry(
  ...
  console.log("[pitch submitEntry]", { gameId, round, txHash });
  return txHash;
}

export async function finalizePitches(
  callerAddress: string,
  gameId: string,
  roundsMeta: PitchRoundMeta[],
  allAddresses: string[],
  ...
  console.log(`[finalizePitches] entries seen on-chain: ${lastSeen}/${expectedTotal}`);

  const txHash = await client.writeContract({
    address: CONTRACT(),
    functionName: "finalize_pitches",
    args: [gameId, JSON.stringify(roundsMeta), currentWeek],
    value: BigInt(0),
    leaderOnly: true,
  });
  console.log("[finalizePitches] tx:", txHash);
  ...
  console.warn("[finalizePitches] Could not extract scores — using XP snapshot");
  ...
  const combo = roundsMeta[0]?.combo ?? "";
  const results: PitchResult[] = ranked.map((r, i) => ({
    address: r.address,
    investability: 0,
    originality: 0,
    buzzword_density: 0,
  ...
  return { combo, results, winner: results[0]?.address ?? "" };
}
