...
import type { PredictionResult, PredictionJudgingResult } from "@/lib/prediction-genlayer";
...
function Card({ r, rank, isMe, delay }: { r: PredictionResult; rank: number; isMe: boolean; delay: number }) {
  ...
  return (
    <div className={`w-full max-w-lg transition-all duration-700 ${show ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
      <div className={`relative px-6 py-5 rounded-xl border ${isMe ? "border-[#22d3ee]/30 bg-[#22d3ee]/[0.04]" : "bg-white/[0.02] border-white/[0.06]"}`} style={rank === 1 ? { boxShadow: `0 0 40px ${color}15` } : {}}>
        ...
        <div className="space-y-2 mb-4">
          <Bar label="Plausible" value={r.plausibility} color="#a855f7" delay={delay + 200} />
          <Bar label="Original" value={r.originality} color="#22d3ee" delay={delay + 400} />
          <Bar label="Hype Level" value={r.hype_level} color="#fbbf24" delay={delay + 600} />
          <Bar label="Happens?" value={r.would_actually_happen} color="#84cc16" delay={delay + 800} />
        </div>
...
export default function PredictionResults({ result, onPlayAgain, onHome }: { result: PredictionJudgingResult; onPlayAgain: () => void; onHome: () => void }) {
    ...
    <div className="min-h-screen flex flex-col items-center py-12 px-4 relative">
      ...
      <div className="relative z-10 w-full max-w-lg flex flex-col items-center">
        <div className={`text-center mb-10 transition-all duration-700 ${show ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-4"}`}>
          <div className="text-[10px] font-mono text-white/30 tracking-[0.6em] uppercase mb-3">The Oracle Has Spoken</div>
          <h1 className="text-4xl md:text-5xl font-extrabold tracking-tighter"><span style={{ color: COLOR }}>PREDICTION</span><span className="text-white"> RESULTS</span></h1>
        ...
        <div className={`mb-10 px-6 py-3 bg-white/[0.03] border border-white/[0.06] rounded-xl text-center transition-all duration-700 ${showPrompt ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
          <div className="text-[9px] font-mono text-white/25 tracking-wider uppercase mb-1">Topic</div>
          <div className="text-lg font-bold" style={{ color: COLOR }}>🔮 {result.topic}</div>
        </div>
