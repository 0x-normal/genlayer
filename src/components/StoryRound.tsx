...
import { pickStoryPrompt } from "@/data/story-prompts";
import { submitEntry, type StoryRoundMeta } from "@/lib/story-genlayer";
...
interface Props {
  ...
  totalRounds: number;
  gameId: string;
  onAllSubmitted: (roundsMeta: StoryRoundMeta[], allAddresses: string[]) => void;
}

export default function StoryRound({ network, players, isHost, duration, totalRounds, gameId, onAllSubmitted }: Props) {
  ...
  const allRoundsRef = useRef<StoryRoundMeta[]>([]);
  ...
  const handleMessage = useCallback((msg: PitchMessage) => {
    ...
    if (msg.type === "ALL_PITCHES" && !hasAdvancedRef.current) {
      ...
      const data = msg.payload as { roundsMeta: StoryRoundMeta[]; allAddresses: string[] };
  ...
  const doSubmit = (auto = false) => {
    ...
    setSubmitted(true);
    const finalText = text.trim() || (auto ? "(no story submitted)" : "");
    if (!finalText) return;

    submitEntry(address, gameId, currentRound, finalText).catch((e) => {
      console.error("[StoryRound] submitEntry failed:", e);
    });
  ...
  useEffect(() => {
    ...
    const roundData: StoryRoundMeta = { round: currentRound, prompt_combo: combo };
    const updatedRounds = [...allRoundsRef.current, roundData];
    allRoundsRef.current = updatedRounds;

    if (currentRound < totalRounds) {
      const nextRound = currentRound + 1;
      const nextCombo = pickStoryPrompt();
    ...
    <div className="min-h-screen flex flex-col items-center justify-center relative px-4">
      ...
      <div className="relative z-10 w-full max-w-2xl">
        ...
        <div className="flex justify-between items-start mb-8">
          <div>
            <div className="text-[10px] font-mono text-white/30 tracking-[0.4em] uppercase mb-1">The Prompt</div>
            <h2 className="text-2xl md:text-3xl font-extrabold tracking-tight">
              {combo ? <><span style={{ color: COLOR }}>{"📖 "}</span><span className="text-white">{combo}</span></> : <span className="text-white/20 animate-pulse">Loading prompt...</span>}
            </h2>
            <p className="text-xs font-mono text-white/25 mt-1">Tell a complete story in a single sentence</p>
          </div>
        ...
        {!submitted ? (
          ...
          <div className="mb-6 px-6 py-8 bg-white/[0.02] border border-[#84cc16]/20 rounded-xl text-center">
            <div className="text-[#84cc16] text-lg font-bold mb-2">✓ Story Submitted</div>
            <div className="text-xs font-mono text-white/30 tracking-wider">
              {currentRound < totalRounds ? `Waiting for others... Round ${currentRound + 1} coming up.` : "Waiting for other authors..."}
            </div>
          </div>
        )}
