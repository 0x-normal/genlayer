...
import { pickCategory } from "@/data/joke-categories";
import { submitEntry, type JokeRoundMeta } from "@/lib/joke-genlayer";
...
interface Props {
  ...
  totalRounds: number;
  gameId: string;
  onAllSubmitted: (roundsMeta: JokeRoundMeta[], allAddresses: string[]) => void;
}

export default function JokeRound({ network, players, isHost, duration, totalRounds, gameId, onAllSubmitted }: Props) {
  ...
  const allRoundsRef = useRef<JokeRoundMeta[]>([]);
  ...
  const handleMessage = useCallback((msg: PitchMessage) => {
    ...
    if (msg.type === "ALL_PITCHES" && !hasAdvancedRef.current) {
      ...
      const data = msg.payload as { roundsMeta: JokeRoundMeta[]; allAddresses: string[] };
  ...
  const doSubmit = (auto = false) => {
    ...
    setSubmitted(true);
    const finalText = text.trim() || (auto ? "(no joke submitted)" : "");
    if (!finalText) return;

    // Fire-and-forget on-chain submission. Each player's wallet is the
    // authoritative author — host can no longer impersonate.
    submitEntry(address, gameId, currentRound, finalText).catch((e) => {
      console.error("[JokeRound] submitEntry failed:", e);
    });

    // Notify peers we've submitted (no payload other than address).
    network.broadcast({ type: "PITCH_SUBMIT", payload: { address } });
    submittedRef.current = new Set([...submittedRef.current, address]);
    setSubmittedPlayers(new Set(submittedRef.current));
  };

  // Host advances rounds once every player has notified completion.
  useEffect(() => {
    ...
    <div className="min-h-screen flex flex-col items-center justify-center relative px-4">
      ...
      <div className="relative z-10 w-full max-w-2xl">
        ...
        <div className="flex justify-between items-start mb-8">
          <div>
            <div className="text-[10px] font-mono text-white/30 tracking-[0.4em] uppercase mb-1">Joke Category</div>
            <h2 className="text-2xl md:text-3xl font-extrabold tracking-tight">
              {combo ? <><span style={{ color: COLOR }}>{"😂 "}</span><span className="text-white">{combo}</span></> : <span className="text-white/20 animate-pulse">Picking category...</span>}
            </h2>
            <p className="text-xs font-mono text-white/25 mt-1">Tell your best joke in this category</p>
          </div>
        ...
        {!submitted ? (
          ...
          <div className="mb-6 px-6 py-8 bg-white/[0.02] border border-[#84cc16]/20 rounded-xl text-center">
            <div className="text-[#84cc16] text-lg font-bold mb-2">🎤 Joke Submitted</div>
            <div className="text-xs font-mono text-white/30 tracking-wider">
              {currentRound < totalRounds ? `Waiting for others... Round ${currentRound + 1} coming up.` : "Waiting for other comedians..."}
            </div>
          </div>
        )}
