...
import { pickTopic } from "@/data/prediction-topics";
import { submitEntry, type PredictionRoundMeta } from "@/lib/prediction-genlayer";
...
interface Props {
  ...
  totalRounds: number;
  gameId: string;
  onAllSubmitted: (roundsMeta: PredictionRoundMeta[], allAddresses: string[]) => void;
}

export default function PredictionRound({ network, players, isHost, duration, totalRounds, gameId, onAllSubmitted }: Props) {
  ...
  const allRoundsRef = useRef<PredictionRoundMeta[]>([]);
  ...
  const handleMessage = useCallback((msg: PitchMessage) => {
    ...
    if (msg.type === "ALL_PITCHES" && !hasAdvancedRef.current) {
      ...
      const data = msg.payload as { roundsMeta: PredictionRoundMeta[]; allAddresses: string[] };
  ...
  const doSubmit = (auto = false) => {
    ...
    setSubmitted(true);
    const finalText = text.trim() || (auto ? "(no prediction submitted)" : "");
    if (!finalText) return;

    submitEntry(address, gameId, currentRound, finalText).catch((e) => {
      console.error("[PredictionRound] submitEntry failed:", e);
    });
    ...
    <div className="min-h-screen flex flex-col items-center justify-center relative px-4">
      ...
      <div className="relative z-10 w-full max-w-2xl">
        ...
        <div className="flex justify-between items-start mb-8">
          <div>
            <div className="text-[10px] font-mono text-white/30 tracking-[0.4em] uppercase mb-1">Predict The Future</div>
            <h2 className="text-2xl md:text-3xl font-extrabold tracking-tight">
              {combo ? <><span style={{ color: COLOR }}>{"🔮 "}</span><span className="text-white">{combo}</span></> : <span className="text-white/20 animate-pulse">Loading topic...</span>}
            </h2>
            <p className="text-xs font-mono text-white/25 mt-1">Submit your wildest prediction on this topic</p>
          </div>
        ...
        {!submitted ? (
          <div className="mb-6">
            <textarea value={text} onChange={(e) => setText(e.target.value.slice(0, MAX_CHARS))}
              placeholder={`Topic: "${combo}"\n\nDrop your boldest prediction. Be specific, be wild, be right.\n\nRound ${currentRound}/${totalRounds} · ${Math.round(duration / 60)} min on the clock.`}
          ...
          <div className="mb-6 px-6 py-8 bg-white/[0.02] border border-[#84cc16]/20 rounded-xl text-center">
            <div className="text-[#84cc16] text-lg font-bold mb-2">✓ Prediction Locked In</div>
            <div className="text-xs font-mono text-white/30 tracking-wider">
              {currentRound < totalRounds ? `Waiting for others... Round ${currentRound + 1} coming up.` : "Waiting for other oracles..."}
            </div>
          </div>
        )}
