...
class StoryWars(gl.Contract):
    ...
    @gl.public.write
    def finalize_stories(self, game_id: str, rounds_meta_json: str, current_week: int) -> str:
        _validate_game_id(game_id)
        if self.finalized.get(game_id, u256(0)) != u256(0):
            raise Exception("game already finalized")
        ...
        for e in entries:
            ...
            if rn < 1 or not addr or not text:
                continue
            by_round.setdefault(rn, []).append({"address": addr, "story": text})
        ...
        for meta in rounds_meta:
            ...
            rounds.append({
                "round": rn,
                "prompt_combo": str(meta.get("prompt_combo", "")),
                "stories": by_round.get(rn, []),
            })
        ...
        rounds_text = ""
        for r in rounds:
            rounds_text += f"\n--- Round {r['round']}: {r['prompt_combo']} ---\n"
            for p in r["stories"]:
                rounds_text += f"  {p['address']}: \"{p['story']}\"\n"

        prompt = f"""You are a ruthless literary critic judging a {num_rounds}-round flash-fiction battle on the GenLayer blockchain.

Players competed across {num_rounds} rounds. Judge each player on their OVERALL storytelling across ALL rounds.

{rounds_text}

Score EACH of the {num_players} players on their overall performance across all rounds (0-10 each):
1. storytelling - Beginning, middle, and end in too few words
2. economy_of_words - Maximum impact per syllable
3. emotion - Did it actually make you feel anything?
4. twist_quality - Was there a real surprise or just filler?
...
{{"results": [{{"address": "0xFULL_ADDRESS", "storytelling": 7, "economy_of_words": 7, "emotion": 7, "twist_quality": 7, "total": 28, "roast": "short literary-critic roast of their prose"}}], "winner": "0xWINNER_ADDRESS"}}"""
        ...
        def leader_fn():
            ...
            for r in results:
                v1 = max(0, min(10, int(round(float(r.get("storytelling", 5))))))
                r["storytelling"] = v1
                v2 = max(0, min(10, int(round(float(r.get("economy_of_words", 5))))))
                r["economy_of_words"] = v2
                v3 = max(0, min(10, int(round(float(r.get("emotion", 5))))))
                r["emotion"] = v3
                v4 = max(0, min(10, int(round(float(r.get("twist_quality", 5))))))
                r["twist_quality"] = v4
                r["total"] = v1 + v2 + v3 + v4
        ...
        def validator_fn(leader_result) -> bool:
            ...
            for r in results:
                ...
                if not isinstance(r.get("address"), str) or len(r.get("address", "")) < 10:
                    return False
                for key in ("storytelling", "economy_of_words", "emotion", "twist_quality"):
                    val = r.get(key)
