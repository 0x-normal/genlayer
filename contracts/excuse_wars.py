...
class ExcuseWars(gl.Contract):
    ...
    @gl.public.write
    def finalize_excuses(self, game_id: str, rounds_meta_json: str, current_week: int) -> str:
        _validate_game_id(game_id)
        if self.finalized.get(game_id, u256(0)) != u256(0):
            raise Exception("game already finalized")
        ...
        for e in entries:
            ...
            if rn < 1 or not addr or not text:
                continue
            by_round.setdefault(rn, []).append({"address": addr, "excuse": text})
        ...
        for meta in rounds_meta:
            ...
            rounds.append({
                "round": rn,
                "crime": str(meta.get("crime", "")),
                "excuses": by_round.get(rn, []),
            })
        ...
        rounds_text = ""
        for r in rounds:
            rounds_text += f"\n--- Round {r['round']}: {r['crime']} ---\n"
            for p in r["excuses"]:
                rounds_text += f"  {p['address']}: \"{p['excuse']}\"\n"

        prompt = f"""You are a skeptical AI courtroom judge judging a {num_rounds}-round excuse battle on the GenLayer blockchain.

Players competed across {num_rounds} rounds. Judge each player on their OVERALL excuse-making performance across ALL rounds.

{rounds_text}

Score EACH of the {num_players} players on their overall performance across all rounds (0-10 each):
1. believability - Could a tired teacher actually buy this?
2. creativity - Fresh angles, unexpected scapegoats, narrative twists
3. desperation - Pure unhinged energy of someone caught red-handed
4. legal_viability - Would this hold up in small claims court?
...
{{"results": [{{"address": "0xFULL_ADDRESS", "believability": 7, "creativity": 7, "desperation": 7, "legal_viability": 7, "total": 28, "roast": "short witty roast of their excuse style"}}], "winner": "0xWINNER_ADDRESS"}}"""
        ...
        def leader_fn():
            ...
            for r in results:
                v1 = max(0, min(10, int(round(float(r.get("believability", 5))))))
                r["believability"] = v1
                v2 = max(0, min(10, int(round(float(r.get("creativity", 5))))))
                r["creativity"] = v2
                v3 = max(0, min(10, int(round(float(r.get("desperation", 5))))))
                r["desperation"] = v3
                v4 = max(0, min(10, int(round(float(r.get("legal_viability", 5))))))
                r["legal_viability"] = v4
                r["total"] = v1 + v2 + v3 + v4
        ...
        def validator_fn(leader_result) -> bool:
            ...
            for r in results:
                ...
                if not isinstance(r.get("address"), str) or len(r.get("address", "")) < 10:
                    return False
                for key in ("believability", "creativity", "desperation", "legal_viability"):
                    val = r.get(key)
