...
class PitchWars(gl.Contract):
    ...
    @gl.public.write
    def finalize_pitches(self, game_id: str, rounds_meta_json: str, current_week: int) -> str:
        _validate_game_id(game_id)
        if self.finalized.get(game_id, u256(0)) != u256(0):
            raise Exception("game already finalized")
        ...
        for e in entries:
            ...
            if rn < 1 or not addr or not text:
                continue
            by_round.setdefault(rn, []).append({"address": addr, "pitch": text})
        ...
        for meta in rounds_meta:
            ...
            rounds.append({
                "round": rn,
                "combo": str(meta.get("combo", "")),
                "pitches": by_round.get(rn, []),
            })
        ...
        rounds_text = ""
        for r in rounds:
            rounds_text += f"\n--- Round {r['round']}: {r['combo']} ---\n"
            for p in r["pitches"]:
                rounds_text += f"  {p['address']}: \"{p['pitch']}\"\n"

        prompt = f"""You are a ruthless venture capital AI panel judging a {num_rounds}-round absurd-startup pitch battle on the GenLayer blockchain.

Players pitched startups based on random concept combinations. Judge each player on their OVERALL pitching across ALL rounds.

{rounds_text}

Score EACH of the {num_players} players on their overall performance across all rounds (0-10 each):
1. investability - Would a real (degen) VC actually fund this?
2. originality - Fresh angles, unique takes - or recycled SaaS slop?
3. buzzword_density - Are they speaking the language of VCs (synergy, AI, web3, etc.)
4. humor - Did the pitch make you laugh on the way to a term sheet?
...
{{"results": [{{"address": "0xFULL_ADDRESS", "investability": 7, "originality": 7, "buzzword_density": 7, "humor": 7, "total": 30, "roast": "short VC-style roast on their overall pitches"}}], "winner": "0xWINNER_ADDRESS"}}"""
        ...
        def leader_fn():
            ...
            for r in results:
                investability = max(0, min(10, int(round(float(r.get("investability", 5))))))
                r["investability"] = investability
                originality = max(0, min(10, int(round(float(r.get("originality", 5))))))
                r["originality"] = originality
                buzzword_density = max(0, min(10, int(round(float(r.get("buzzword_density", 5))))))
                r["buzzword_density"] = buzzword_density
                humor = max(0, min(10, int(round(float(r.get("humor", 5))))))
                r["humor"] = humor
                r["total"] = investability + originality + buzzword_density + humor
        ...
        def validator_fn(leader_result) -> bool:
            ...
            for r in results:
                ...
                if not isinstance(r.get("address"), str) or len(r.get("address", "")) < 10:
                    return False
                for key in ('investability', 'originality', 'buzzword_density', 'humor'):
                    val = r.get(key)
