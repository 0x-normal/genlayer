...
class JokeWars(gl.Contract):
    ...
    @gl.public.write
    def finalize_jokes(self, game_id: str, rounds_meta_json: str, current_week: int) -> str:
        _validate_game_id(game_id)
        if self.finalized.get(game_id, u256(0)) != u256(0):
            raise Exception("game already finalized")
        ...
        for e in entries:
            ...
            if rn < 1 or not addr or not text:
                continue
            by_round.setdefault(rn, []).append({"address": addr, "joke": text})
        ...
        for r in rounds:
            ...
            for p in r["jokes"]:
                rounds_text += f"  {p['address']}: \"{p['joke']}\"\n"

        prompt = f"""You are a brutally honest comedy judge judging a {num_rounds}-round joke battle on the GenLayer blockchain.

Players competed across {num_rounds} rounds. Judge each player on their OVERALL comedy performance across ALL rounds.

{rounds_text}

Score EACH of the {num_players} players on their overall performance across all rounds (0-10 each):
1. funniness - Overall funniness across all their jokes
2. originality - How original and fresh were their jokes across rounds?
3. timing - Consistent setup/punchline quality across the game
4. groans - Groan-worthiness - for puns/dad jokes this is the highest honour
...
{{"results": [{{"address": "0xFULL_ADDRESS", "funniness": 7, "originality": 7, "timing": 7, "groans": 7, "total": 28, "roast": "short comedy critic roast on their overall game"}}], "winner": "0xWINNER_ADDRESS"}}"""
        ...
        def leader_fn():
            ...
            for r in results:
                v1 = max(0, min(10, int(round(float(r.get("funniness", 5))))))
                r["funniness"] = v1
                v2 = max(0, min(10, int(round(float(r.get("originality", 5))))))
                r["originality"] = v2
                v3 = max(0, min(10, int(round(float(r.get("timing", 5))))))
                r["timing"] = v3
                v4 = max(0, min(10, int(round(float(r.get("groans", 5))))))
                r["groans"] = v4
                r["total"] = v1 + v2 + v3 + v4
        ...
        def validator_fn(leader_result) -> bool:
            ...
            for r in results:
                ...
                if not isinstance(r.get("address"), str) or len(r.get("address", "")) < 10:
                    return False
                for key in ("funniness", "originality", "timing", "groans"):
                    val = r.get(key)
